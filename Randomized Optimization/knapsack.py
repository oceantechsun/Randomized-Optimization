...
for y in range(10):
    ...
    for x in range(5):
        start = time.time()
        opt = ml.DiscreteOpt((y*6)+6, fitness[y])
...
for y in range(10):
    ...
    for x in range(5):
        ...
        best_state_hill, best_fitness_hill, curve_hill = ml.random_hill_climb(opt, max_iters=5, restarts = 1, curve=True, random_state = 2)
...
plt.legend(loc="best")
plt.savefig("nap_total_iters.png")
plt.clf()
...
plt.legend(loc="best")
plt.savefig("nap_time_iters.png")
plt.clf()